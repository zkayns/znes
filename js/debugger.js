...
function Debugger(nes, ctx) {
  ...
  this.updateDebugView = function() {
    ...
    let line = (""+this.nes.ppu.line).padStart(3, "0");
    let dot = (""+this.nes.ppu.dot).padStart(3, "0");
  ...
  this.drawRam = function() {
    ...
    ev.style["text-align"]="center";
    ev.textContent="";
    let ramBasePos=this.ramScroll;
    for (let r=ramBasePos; r<ramBasePos+32; r++) {
      let str=`${this.nes.getWordRep(r*16)}: `;
      for (let c=0; c<16; c++) str+=`${this.nes.getByteRep(this.nes.peak(r*16+c))} `;
      ev.textContent+=`${str}\n`;
    }
  }

  this.drawDisassembly = function() {
    if (this.lastDis==this.disScroll) return;
    let ev=el("dtextoutput");
    ev.style["text-align"]="left";
    ev.textContent="";
    this.disScroll=this.nes.cpu.br[0]-16;
    let adr=this.disScroll, lines=0, firstData=true;
    while(adr<65536) {
      let op=this.nes.peak(adr);
      let length=this.opLengths[this.nes.cpu.addressingModes[op]];
      let isOpcode=adr>=32768;
      let pcP=adr===this.nes.cpu.br[0];;
      if(isOpcode) {
        ev.innerHTML+=`\t<span style='background-color: ${this.modeColors[this.nes.cpu.addressingModes[op]]};'>${pcP?"<strong>>":" "} ${this.nes.getWordRep(adr)}: ${this.instrStr(adr)}</span>\n`;
        adr+=length;
        firstData=true;
        lines++;
      } else {
    ...
    for (let i=lines; i<32; i++) ev.textContent+="\n";
    this.lastDis=adr;
  }

  this.drawNametables = function() {
    let imgData=this.ctx.createImageData(512, 480);
    for (let x=0; x<64; x++) for (let y=0; y<60; y++) {
      ry=y+(y>=30?2:0);
      let tileNumAdr=8192+(ry>31?2048:0)+(x>31?1024:0);
      tileNumAdr+=((ry&31)<<5)+(x&31);
      let tileNum=this.nes.mapper.ppuPeak(tileNumAdr);
      let attAdr=9152+(ry>31?2048:0)+(x>31?1024:0);
      attAdr+=((ry&28)<<1)+((x&28)>>2);
  ...
  this.drawPatternsPals = function() {
    let imgData=this.ctx.createImageData(512, 480);
    // left table
    for (let x=0; x<16; x++) for (let y=0; y<16; y++) this.drawTile(imgData, x*8, y*8, y*16+x, 0);
    // right table
    for (let x=0; x<16; x++) for(let y=0; y<16; y++) this.drawTile(imgData, 128+x*8, y*8, 256+y*16+x, 0);
    this.ctx.putImageData(imgData, 0, 0);
    // draw palette
    for (let i=0; i<16; i++) {
      let col=this.nes.ppu.nesPal[this.nes.ppu.readPalette(i)&63];
      ctx.fillStyle=`rgba(${col[0]}, ${col[1]}, ${col[2]}, 1)`;
      ctx.fillRect(i*16, 128, 16, 16);
      col=this.nes.ppu.nesPal[this.nes.ppu.readPalette(i+16)&63];
      ctx.fillStyle=`rgba(${col[0]}, ${col[1]}, ${col[2]}, 1)`;
  ...
  this.drawTile = function(imgData, x, y, num, col) {
    for(let i=0; i<8; i++) {
      ...
      for(let j=0; j<8; j++) {
        ...
        let color=this.nes.ppu.nesPal[this.nes.ppu.readPalette(pind)&63];
  ...
  this.instrStr = function(adr) {
    ...
    let i1=this.nes.peak((pc+1)&65535);
    let i2=i1|(this.nes.peak((pc+2)&65535)<<8);
    let adrMode=this.nes.cpu.addressingModes[opcode];
    let opName=this.opNames[opcode];
    let relVal=i1>127?i1-256:i1;
    relVal += pc + 2;
    switch(adrMode) {
