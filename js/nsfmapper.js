function NsfMapper(data, loadAdr, banked, banks) {
  ...
  this.reset=function() {
    ...
    if(this.banked) {
      ...
      for(let i=this.loadAdr; i<this.romData.length; i++) {
        if(0x80+i-this.loadAdr>=this.data.length) break;
  ...
  this.write = function(adr, val) {
    if (adr>=0x5ff8) {
      if (adr<0x6000) return this.banks[adr-0x5ff8]=val%this.maxBanks;
      if (adr<0x8000) return this.prgRam[adr&0x1fff]=val;
    };
    return;
  }
}