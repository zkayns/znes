function Nes() {
  ...
  this.stateVersion = 1;
  // ram
  this.realRam = new Uint8Array(0x800);
  this.ram=new Proxy(this.realRam, {
    get(t, p) {
      return ramLocks[p]??t[p];
    },
    set(t, p, v) {
      t[p]=v;
    }
  ...
  this.reset = function(hard) {
    if(hard) this.realRam=new Uint8Array(0x800);
    this.cpu.reset();
    this.ppu.reset();
    this.apu.reset();
  ...
  this.loadRom = function(rom) {
    ...
    log(`Loaded ${this.mapper.name} rom: ${this.mapper.h.banks} PRG bank(s), ${this.mapper.h.chrBanks} CHR bank(s)`);
    return true;
  }
  ...
  this.parseHeader = function(rom) {
    let o={
      banks: rom[4],
      chrBanks: rom[5],
      mapper: (rom[6]>>4)|(rom[7]&16),
      verticalMirroring: (rom[6]&1)>0,
      battery: (rom[6]&2)>0,
      trainer: (rom[6]&4)>0,
      fourScreen: (rom[6]&8)>0,
    };
    o["base"]=16+(o.trainer?512:0);
    o["chrBase"]=o.base+16384*o.banks;
    o["prgAnd"]=o.banks*16384-1;
    o["chrAnd"]=o.chrBanks===0?0x1fff:o.chrBanks*8192-1;
    o["saveVars"]=Object.keys(o);
    return o;
  }
  ...
  this.setButtonPressed = function(player, button) {
    this[`currentControl${player}State`]|=(1<<button);
  }

  this.setButtonReleased = function(player, button) {
    this[`currentControl${player}State`]&=(~(1<<button))&255;
  }
  ...
  this.getState=function() {
    ...
    final["cpu"]=this.getObjState(this.cpu);
    final["ppu"]=this.getObjState(this.ppu);
    final["apu"]=this.getObjState(this.apu);
    final["mapper"]=this.getObjState(this.mapper);
    final["header"]=this.getObjState(this.mapper.h);
  ...
  this.getObjState=function(obj) {
    ...
    for(let i=0; i<obj.saveVars.length; i++) {
      ...
      let val=obj[name];
      if(val instanceof Uint8Array || val instanceof Uint16Array) ret[name]=Array.prototype.slice.call(val);
      else ret[name]=val;
    }
    return ret;
  }
  ...
  this.setObjState=function(obj, save) {
    for(let i=0; i<obj.saveVars.length; i++) {
      let name=obj.saveVars[i];
      let val=obj[name];
      if(val instanceof Uint8Array) obj[name]=new Uint8Array(save[name]);
      else if(val instanceof Uint16Array) obj[name]=new Uint16Array(save[name]);
      else obj[name]=save[name];
    }
  }
